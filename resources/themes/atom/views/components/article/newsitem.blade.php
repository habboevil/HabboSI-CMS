...
<a href="{{ route('community.articles.show', $article) }}" class="hover:scale-[110%] w-full h-[14em] flex flex-col justify-between rounded-[7px] ease-in-out duration-300 !bg-[-200px_-2px]" style="box-shadow: 0 8px 5px rgba(0, 0, 0, .1);background-image: url({{ Storage::exists($article->image) ? Storage::url($article->image) : $article->image }});">
