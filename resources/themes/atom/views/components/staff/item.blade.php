...
<x-card.base badge="{{ $permission->badge }}" title="{{ $permission->rank_name }}" subtitle="{{ $permission->job_description }}" icon-color="{{ $permission->staff_color }}">
        ...
        @forelse ($permission->users as $user)
            <a href="{{ route('profiles', $user->username) }}" class="w-full bg-[#0000001a] relative overflow-hidden flex items-center h-[100px] rounded-[5px] hover:-translate-y-2 duration-300">
                ...
                <div class="flex flex-col justify-center py-2 px-5 truncate">
                    ...
                    <p class="text-white font-light opacity-60 italic">{{ $user->aufgabe }}</p>
                </div>
